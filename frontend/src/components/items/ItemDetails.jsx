...
const ItemDetails = ({ item, setItem }) => {
	...
	const editHandler = async (event) => {
		...
		const dateInput = document.getElementById(`duedate-${item._id}`)
		const dateValid = dateInput.validity.valid
		if (!dateValid) dateChange = true
		...
		try {
			const response = await api.patch(`/items/${item._id}`, { title, dueDate: validDueDate, completed, note }, { headers })
